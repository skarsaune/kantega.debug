...
var Simple = (function(Simple) {
   ...
   */
  Simple.log = Logger.get('decentipede');
   ...
   *
   */
  Simple.contextPath = "/decentipede-web/";
   ...
   * run function
   */
  Simple.module = angular.module('decentipede', ['hawtioCore'])
      .config(function($routeProvider) {
        ...
        $routeProvider.
            when('/decentipede', {
              templateUrl: Simple.templatePath + 'decentipede.html'
            });
      });
  ...
  Simple.module.run(function(workspace, viewRegistry, layoutFull) {
    ...
    Core.addCSS(Simple.contextPath + "plugin/css/decentipede.css");
    ...
    workspace.topLevelTabs.push({
      ...
      href: function() { return "#/decentipede"; },
      isActive: function(workspace) { return workspace.isLinkActive("decentipede"); }
  ...
  Simple.SimpleController = function($scope, jolokia) {
    ...
    $scope.diagnoseNullPointers = false;
    $scope.running = false;
    ...
    Core.register(jolokia, $scope, {
        type: 'read', mbean: 'no.kantega.debug:type=DebugAgent',
        arguments: []
      }, onSuccess(renderCentipede));
    
    function renderCentipede(response) {
    	$scope.diagnoseNullPointers = response.value['NullPointerDiagnosed'];
    	$scope.emitWalkbacks = response.value['EmitWalkbacks'];
